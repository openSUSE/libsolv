...
#define CTAG(a, b, c, d) ((unsigned)(((unsigned char)a) << 24) | ((unsigned char)b << 16) | ((unsigned char)c << 8) | ((unsigned char)d))
...
{
  return makeevr(pool, !release || (release[0] == '-' && !release[1]) ? version : join2(&pd->jd, version, "-", release));
}

static inline void
record_share(struct parsedata *pd, Id handle, Id name, Id evr, Id arch)
{
  Repo *repo = pd->repo;
...
static void
process_shares(struct parsedata *pd)
{
  Pool *pool = pd->pool;
  Repo *repo = pd->repo;
      ...
      for (n = repo->start, nn = repo->start + last_found; n < repo->end; n++, nn++)
        {
          ...
          if (found->repo == repo && found->name == pd->share_with[i].name && found->evr == pd->share_with[i].evr && found->arch == pd->share_with[i].arch)
            {
