  ...
  while (io + 2 < in_len)
    {
      ...
      for (tries = 0; try != -1 && tries < 12; tries++)
        {
          if (try < io && in[try] == in[io] && in[try + 1] == in[io + 1])
            {
              mlen = 2;
              mofs = (io - try) - 1;
              break;
          ...
          if (try < io && in[try + mlen] == in[io + mlen])
            {
              ...
              for (;
                   io + this_len < in_len && in[try + this_len] == in[io + this_len]; this_len++)
                ;
#if 0
	      unsigned int testi;
          ...
          if (mlen && mlen < (2048 + 5) && io + 3 < in_len)
            {
              ...
              try
                = htab[hval];
              if (try < io + 1 && in[try] == in[io + 1] && in[try + 1] == in[io + 2])
                {
                  unsigned int this_len;
                  this_len = 2;
                  for (;
                       io + 1 + this_len < in_len && in[try + this_len] == in[io + 1 + this_len];
                       this_len++)
                    ;
                  if (this_len >= mlen)
                    mlen = 0;
  ...
  if (pstart == pend)
  ...
  else
    {
      ...
      for (pnum = pstart; pnum <= pend; pnum++)
        if (store->mapped_at[pnum] == -1 || (pnum > pstart && store->mapped_at[pnum] != store->mapped_at[pnum - 1] + REPOPAGE_BLOBSIZE))
          break;
      if (pnum > pend)
        return store->blob_store + store->mapped_at[pstart];
    }
