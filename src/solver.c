...
static void
prune_weakexluded(Solver *solv, Queue *plist)
{
  int i, j;
  for (i = j = 0; i < plist->count; i++)
    {
      Id p = plist->elements[i];
      if (!MAPTST(&solv->weakexcludemap, p))
        plist->elements[j++] = p;
    }
  if (i != j)
    queue_truncate(plist, j);
...
resolve_weak(Solver *solv, int level, int disablerules, Queue *dq, Queue *dqs, int *rerunp)
{
  ...
  for (i = 1; i < pool->nsolvables; i++)
      ...
      else
	{
	  ...
	  if (solv->weakexcludemap.size && MAPTST(&solv->weakexcludemap, i))
  ...
  if (solv->weakexcludemap.size)
    prune_weakexluded(solv, dq);
...
static void
solver_add_weakexcludes(Solver *solv)
{
  Queue *job = &solv->job;
  Pool *pool = solv->pool;
  int i;
...
for (i = 0; i < job->count; i += 2)
  {
    how = job->elements[i];
    if ((how & SOLVER_JOBMASK) != SOLVER_WEAK_EXCLUDE)
	continue;
    if (!solv->weakexcludemap.size)
	map_grow(&solv->weakexcludemap, pool->nsolvables);
    what = job->elements[i + 1];
    select = how & SOLVER_SELECTMASK;
...
solver_solve(Solver *solv, Queue *job)
{
  ...
  map_zerosize(&solv->weakexcludemap);
  ...
  for (i = 0; i < job->count; i += 2)
	...
	case SOLVER_BLACKLIST:
	  ...
	  hasblacklistjob = 1;
	  break;
        case SOLVER_WEAK_EXCLUDE:
	  POOL_DEBUG(SOLV_DEBUG_JOB, "job: weakexclude %s\n", solver_select2str(pool, select, what));
	  hasweakexcludesjob = 1;
	  break;
	default:
  ...
  if (hasweakexcludesjob)
    solver_add_weakexcludes(solv);
