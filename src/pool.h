...
pool_id2solvable(const Pool *pool, Id p)
{
...
pool_solvid2str(Pool *pool, Id p)
{
...
static inline int
pool_match_nevr(Pool *pool, Solvable *s, Id d)
{
  if (!ISRELDEP(d))
    return d == s->name;
  else
...
static inline Id
pool_whatprovides(Pool *pool, Id d)
{
  if (!ISRELDEP(d))
    {
...
static inline Id *
pool_whatprovides_ptr(Pool *pool, Id d)
{
  ...
  do                                           \
    {                                          \
      if ((pool->debugmask & (type)) != 0)     \
        pool_debug(pool, (type), __VA_ARGS__); \
    }                                          \
  while (0)
